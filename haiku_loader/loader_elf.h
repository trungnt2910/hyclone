...
#define ELFOSABI_TRuint64_t_      10
...
#define EF_CPuint32_t_ 0x00810000
...
typedef struct Elf64_Ehdr_ {
  uint8_t ident[EI_NIDENT_];
  uint8_t type[2];       // u16
  uint8_t machine[2];    // u16
  uint8_t version[4];    // u32
  uint8_t entry[8];      // u64
  uint8_t phoff[8];      // u64
  uint8_t shoff[8];      // u64
  uint8_t flags[4];      // u32
  uint8_t ehsize[2];     // u16
  uint8_t phentsize[2];  // u16
  uint8_t phnum[2];      // u16
  uint8_t shentsize[2];  // u16
  uint8_t shnum[2];      // u16
  uint8_t shstrndx[2];   // u16
} Elf64_Ehdr_;

typedef struct Elf64_Phdr_ {
  uint8_t type[4];    // u32
  uint8_t flags[4];   // u32
  uint8_t offset[8];  // u64
  uint8_t vaddr[8];   // u64
  uint8_t paddr[8];   // u64
  uint8_t filesz[8];  // u64
  uint8_t memsz[8];   // u64
  uint8_t align[8];   // u64
} Elf64_Phdr_;
...
typedef struct Elf64_Shdr_ {
  ...
  uint8_t link[4];       // u32
  uint8_t info[4];       // u32
  uint8_t addralign[8];  // u64
  uint8_t entsize[8];    // u64
} Elf64_Shdr_;
...
typedef struct Elf64_Syminfo_ {
  uint8_t boundto[2];  // u16
  uint8_t flags[2];    // u16
} Elf64_Syminfo_;

typedef struct Elf64_Chdr_ {
  uint8_t type[4];       // u32
  uint8_t reserved[4];   // u32
  uint8_t size[8];       // u64
  uint8_t addralign[8];  // u64
} Elf64_Chdr_;

typedef struct Elf64_Dyn_ {
  uint8_t tag[8];  // int64_t
  uint8_t val[8];  // u64
} Elf64_Dyn_;

typedef struct Elf64_Lib_ {
  uint8_t name[4];        // u32
  uint8_t time_stamp[4];  // u32
  uint8_t checksum[4];    // u32
  uint8_t version[4];     // u32
  uint8_t flags[4];       // u32
} Elf64_Lib_;
...
typedef struct Elf64_Move_ {
  uint8_t value[8];    // u64
  uint8_t info[8];     // u64
  uint8_t poffset[8];  // u64
  uint8_t repeat[2];   // u16
  uint8_t stride[2];   // u16
} Elf64_Move_;

typedef struct Elf64_Nhdr_ {
  uint8_t namesz[4];  // u32
  uint8_t descsz[4];  // u32
  uint8_t type[4];    // u32
} Elf64_Nhdr_;

void CheckElfAddress(const Elf64_Ehdr_ *, size_t, uintptr_t, size_t);
Elf64_Phdr_ *GetElfSegmentHeaderAddress(const Elf64_Ehdr_ *, size_t, uint64_t);
char *GetElfStringTable(const Elf64_Ehdr_ *, size_t);
Elf64_Shdr_ *GetElfSectionHeaderAddress(const Elf64_Ehdr_ *, size_t, uint16_t);
const char *GetElfSectionName(const Elf64_Ehdr_ *, size_t, Elf64_Shdr_ *);
char *GetElfString(const Elf64_Ehdr_ *, size_t, const char *, uint32_t);
...
int64_t GetElfMemorySize(const Elf64_Ehdr_ *, size_t, int64_t *);

static inline uint64_t Read64(const void *addr) { return *(const uint64_t*)addr; }
static inline uint32_t Read32(const void *addr) { return *(const uint32_t*)addr; }
static inline uint16_t Read16(const void *addr) { return *(const uint16_t*)addr; }
