...
public:
    ...
    status_t ReceiveFrom(port_id fromPort,
                         bigtime_t timeout = -1,
                         haiku_port_message_info *messageInfo = NULL);
...
int8 KMessage::GetInt8(const char *name, int8 defaultValue) const
{
    return _GetType(name, B_INT8_TYPE, 0, defaultValue);
}

int8 KMessage::GetInt8(const char *name, int32 index, int8 defaultValue) const
{
    return _GetType(name, B_INT8_TYPE, index, defaultValue);
}

int16 KMessage::GetInt16(const char *name, int16 defaultValue) const
{
    return _GetType(name, B_INT16_TYPE, 0, defaultValue);
}

int16 KMessage::GetInt16(const char *name, int32 index, int16 defaultValue) const
{
                    ...
                    const char *defaultValue) const
{
    ...
    int32 size;
    const char *value;
    if (FindData(name, B_STRING_TYPE, index, (const void **)&value, &size) == B_OK)
    {
        return value;
    }
    return defaultValue;
