...
// We'll implement this in HyClone infrastructure in a separate file.
#define KMESSAGE_CONTAINER_ONLY
// Macro currently used only in unimportant ::Dump() function
...
#if !defined(HAIKU_TARGET_PLATFORM_HAIKU) || defined(_BOOT_MODE) || defined(_LOADER_MODE)
...
_Align(int32 offset)
{
    return (offset + kMessageBufferAlignment - 1) & ~(kMessageBufferAlignment - 1);
}

static inline void *
_Align(void *address, int32 offset = 0)
{
    return (void *)(((addr_t)address + offset + kMessageBufferAlignment - 1) & ~(kMessageBufferAlignment - 1));
}
...
KMessage::SetTo(void *buffer, int32 bufferSize, uint32 what, uint32 flags)
{
    ...
    if ((flags & KMESSAGE_READ_ONLY) != 0 && (flags & KMESSAGE_INIT_FROM_BUFFER) == 0)
    {
        return B_BAD_VALUE;
    }

    // if not initializing from the given buffer, cloning it doesn't make sense
    if ((flags & KMESSAGE_INIT_FROM_BUFFER) == 0 && (flags & KMESSAGE_CLONE_BUFFER) != 0)
    {
        return B_BAD_VALUE;
    }
...
void KMessage::SetWhat(uint32 what)
{
    _Header()->what = what;
}
...
int32 KMessage::BufferCapacity() const
{
    return fBufferCapacity;
}

int32 KMessage::ContentSize() const
{
    return _Header()->size;
}
                    ...
                    KMessageField *field) const
{
    ...
    {
        if ((type == B_ANY_TYPE || field->TypeCode() == type) && strcmp(name, field->Name()) == 0)
        {
            return B_OK;
        }
    }
...
KMessage::GetNextField(KMessageField *field) const
{
    ...
    {
        if ((uint8 *)fieldHeader < (uint8 *)_FirstFieldHeader() || (uint8 *)fieldHeader > (uint8 *)lastField)
        {
            return B_BAD_VALUE;
        }
...
{
    if (!name || type == B_ANY_TYPE || !data || elementSize < 0 || elementCount < 0)
    {
        return B_BAD_VALUE;
    }
    KMessageField field;
                  ...
                  int32 numBytes)
{
    ...
    {
        // field already known
        if (field.TypeCode() != type || !field.HasFixedElementSize() || field.ElementSize() != numBytes)
        {
            return B_BAD_VALUE;
        }
...
int32 KMessage::TargetToken() const
{
...
int32 KMessage::ReplyToken() const
{
    return _Header()->replyToken;
}

void KMessage::SetDeliveryInfo(int32 targetToken, port_id replyPort,
...
void KMessage::Dump(void (*printFunc)(const char *, ...)) const
{
    Header *header = _Header();
    printFunc("KMessage: buffer: %p (size/capacity: %ld/%ld), flags: %#" B_PRIx32 "\n", fBuffer, header->size, fBufferCapacity, fFlags);
...
int32 KMessage::_BufferOffsetFor(const void *data) const
{
    if (!data)
        return -1;
                        ...
                        int32 elementSize, int32 elementCount)
{
    ...
    FieldHeader *fieldHeader = field->_Header();
    FieldHeader *lastField = _LastFieldHeader();
    if (!fieldHeader || fieldHeader != lastField || !data || elementSize < 0 || elementCount < 0)
    {
        return B_BAD_VALUE;
    }
    if (elementCount == 0)
    ...
    if (fieldHeader->HasFixedElementSize())
    {
        ...
        fieldHeader->fieldSize = (uint8 *)address + alignedSize - (uint8 *)fieldHeader;
        return B_OK;
    }
...
KMessage::_InitFromBuffer(bool sizeFromBuffer)
{
    ...
    while (remainingBytes > 0)
    {
        ...
        if (fieldHeader->elementCount < 0)
            return B_BAD_DATA;
        if (fieldHeader->fieldSize < (int)sizeof(FieldHeader) || fieldHeader->fieldSize > remainingBytes)
        {
            return B_BAD_DATA;
        }
        if (fieldHeader->headerSize < (int)sizeof(FieldHeader) || fieldHeader->headerSize > fieldHeader->fieldSize)
        {
            return B_BAD_DATA;
        }
        int32 maxNameLen = data + fieldHeader->headerSize - (uint8 *)fieldHeader->name;
        ...
        {
            // fixed element size
            int32 dataSize = fieldHeader->elementSize * fieldHeader->elementCount;
            fieldSize = (uint8 *)fieldHeader->Data() + dataSize - data;
        }
        else
        {
            // non-fixed element size
            FieldValueHeader *valueHeader = (FieldValueHeader *)fieldHeader->Data();
            for (int32 i = 0; i < fieldHeader->elementCount; i++)
            {
                remainingBytes = (uint8 *)fBuffer + header->size - (uint8 *)valueHeader;
...
void KMessage::_InitBuffer(uint32 what)
{
    Header *header = _Header();
...
int32 KMessage::_CapacityFor(int32 size)
{
    return (size + kMessageReallocChunkSize - 1) / kMessageReallocChunkSize * kMessageReallocChunkSize;
}
...
bool KMessageField::HasFixedElementSize() const
{
    KMessage::FieldHeader *header = _Header();
    return header ? header->HasFixedElementSize() : false;
}

int32 KMessageField::ElementSize() const
{
    KMessage::FieldHeader *header = _Header();
...
int32 KMessageField::CountElements() const
{
    KMessage::FieldHeader *header = _Header();
    return header ? header->elementCount : 0;
}

void KMessageField::SetTo(KMessage *message, int32 headerOffset)
{
    fMessage = message;
